...
shinyServer(
	function(input, output, session) {
	  ...
	  # ---- START OF AYAH'S CODE ----
	  output$ayah_events <- renderTable({seattle_events})
	  
	  output$ayah_table <- renderTable({
	  	pred <- as.vector(sea_predictions()$pct_precip_smooth)
	  	index <- round(as.numeric(input$ayah_day_selection))
	  	pct_prediction <- as.numeric(pred[index])
	  	
	  	if (pct_prediction < 0.3){
	  		return(will_it_rain[1,])
	  	} else if (pct_prediction < 0.6){
	  		return(will_it_rain[2,])
	  	} else {
	  		return(will_it_rain[3,])
	  	}
	  })
	  
	  output$ayah_temperature <- renderPlot({ayah_temp})
	  
	  output$ayah_populations <- renderPlot({ayah_pop})
	  # ---- END OF AYAH'S CODE ----
